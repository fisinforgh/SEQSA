...
then
	...
	warnInfo=$(echo " "
		   ...
		   echo " We recommend install precompiled version. Go to 'Download a pre-compiled binary distribution' section for further info."
		   echo " ")
	zenity --info --text="$warnInfo" --ok-label="Exit install" #Cuadro de salida para cuando la instalacion falla
	echo "bye"
	sleep 1 #Simula tiempo de espera entre operaciones 
	exit 1
    fi
fi
    
#----------------------------------------------------------------------------------------------------------------------------#
#Inicia el contador y muestra otra ventana
if [ $cont -eq 1 ]; then
    infoINS1=$(
	echo "ROOT System install found it!"
	echo "Please click CONTINUE to select SoftEduQSA Installation Folder"
	echo " "
	command -v $_ROOT #Verifica si el comando se encuentra en el sistema, la opcion -v hace que busque un programa en especial
	$pathROOTEXE -q
	...
	echo "$pathROOTCONFIG"
	echo " ")
    echo "$infoINS1" | (zenity --text-info --title="SoftEduQSA install" --width=800 --height=400 --ok-label='CONTINUE' --cancel-label='Exit install') #Cuadro para pasar a la instalacion

#Comando para cuando no encuentra Root
    if [ $? -eq 1 ]; then
	echo "Exit"
	exit 2
    fi   
...
###Activar cambiando el link (wget -O $dirPATH/$tarFILE https://raw.githubusercontent.com/fisinforgh/) | zenity --progress --width=400 --height=200 --auto-close \ ###Barra de progreso
...
if [ -f "$FILE" ]; then
    rm $FILE
    ($pathROOTCLING -f SoftEduQSADict.cxx -c SoftEduQSA.h SoftEduQSALinkDef.h
     g++ -o SoftEduQSA SoftEduQSA.cxx SoftEduQSADict.cxx `$pathROOTCONFIG --cflags --glibs` -lGeom) | zenity --progress --width=800 --height=400 --auto-close \
												    --title="SoftEduQSA Install" \
