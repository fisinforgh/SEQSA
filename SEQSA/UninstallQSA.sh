...
messageUNIS=$(echo "Welcome to the SEQSA UNINSTALL wizard"
	      echo "You are about to uninstall SEQSA software"
	      echo "If you are completely sure, please clik on UNINSTALL"
	      )

echo "$messageUNIS" | (zenity --text-info --title="SEQSA uninstall" --width=800 --height=400 --ok-label='UNINSTALL' --cancel-label='Exit')
if [ $? -eq 1 ]; then
    echo "Exit"
    exit 2
fi
...
#----------------------------------------------------------------------------------------------------------------------------#
(zenity --info --text="Please click CONTINUE to select \"Desktop\" Folder to UNISTALL SEQSA desktop's icon" --title="SEQSA UNINSTALL desktop icon" --ok-label='CONTINUE' --width=800 --height=400 )

dirDESK=$(zenity --file-selection   --directory --title="SEQSA UNINSTALL choose a Desktop Folder" --width=800 --height=400)

manUNIS=$( echo "Remove SEQSA manually:"
	   ...
	   echo "export PATH=\"/home/fisinfor/test_v2/SEQSA:\$PATH")
